...
- WHITESPACE: use it aggressively and intentionally — spacing groups related items and separates unrelated ones (proximity IS hierarchy). Visual hierarchy is built with size, spacing, and contrast working together, never with color alone.
...
- EFFECTS MATCH THE STYLE: shadows, blur, and border-radius must all speak the chosen design system's language — soft/glass styles get blur and translucency, brutalist/editorial styles get hard edges and flat rules, warm/organic styles get soft large radii. Define ONE elevation scale (e.g. small/medium/large shadow tokens) and reuse it consistently — never invent a random one-off shadow per card. Border-radius is consistent page-wide.
- NAV clean and light.
...
- Micro-details that signal quality: consistent 8px spacing system, one consistent desktop container max-width (~1140-1280px) applied uniformly across sections, a deliberate z-index scale (e.g. 0/10/20/100) instead of arbitrary values, letterspaced uppercase labels above headings, oversized section numerals or stat numerals where fitting, hanging quotes on pull-quotes, a real footer with columns (brand, links, hours, contact).
