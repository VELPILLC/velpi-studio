...
STRUCTURE MANIFEST — MANDATORY, AND IT MUST BE AN HTML COMMENT: immediately after <!DOCTYPE html> and BEFORE <html>, add ONE HTML comment — literally <!-- STRUCTURE: ... --> — mapping every section, in order, to the blueprint id it implements. It goes in the document itself, NEVER as a CSS comment inside the <style> tag (a /* STRUCTURE MANIFEST */ block in CSS does not satisfy this and is a failed output): the manifest is machine-checked at the top of the document, and tooling looking for the HTML comment will not find it anywhere else (e.g. <!-- STRUCTURE: nav=daisyui--navbar | hero=velpi--hero--split-editorial-masthead | services=hyperui--feature-grid | reviews=velpi--testimonials--overlap-quote-photo | contact=velpi--cta--floating-dock-panel | footer=velpi--footer--editorial-grid-sitemap -->). This manifest is the verifiable contract that a regeneration of the same business can be checked against — it must list every section in SECTION ORDER plus the nav, each with the exact blueprint id assigned below.
...
PREMIUM TECHNIQUES — make the page feel ALIVE, like a high-end agency built it:
...
- ONE self-contained file: ALL CSS in a single <style> tag. Google Fonts loaded via @import at the TOP of that <style> tag (never a <link> tag) — @import survives when the code is pasted into a GoHighLevel custom-code element. Pinned CDN <script> tags are the ONE permitted external dependency (see the ADVANCED OUTPUT rules below); there are never external stylesheets, external images, or fetched data files.
...
- ADVANCED OUTPUT — JAVASCRIPT AND REAL WebGL ARE ALLOWED AND EXPECTED. This is the single biggest thing separating a premium build from a template, and it is not optional polish. It survives GoHighLevel only under these rules:
  * SCRIPT PLACEMENT — THE ONE RULE THAT SILENTLY BREAKS EVERYTHING IF IGNORED: every <script> is a SIBLING of the wrapper, placed AFTER its closing </div>, NEVER nested inside it. GoHighLevel mangles a <script> nested in a div. The document shape is exactly: <div class="velpi-page"> ... </div> then the CDN <script src> tags then your own inline <script>. No exceptions, no inline event handler attributes (onclick=...) as a workaround.
  * CDN libraries are allowed at an EXACT PINNED VERSION — never @latest, never an unversioned or "latest" path. Three.js is loaded as <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>. Do not load a module/ESM build, do not use import maps, and do not assume any bundler exists — plain classic scripts and the global THREE only.
  * ALL 3D IS PROCEDURAL — THE GEOMETRY IS WRITTEN IN CODE. Never load a .glb/.gltf/.fbx/.obj model, never fetch an external texture, HDR, or image file. Build every scene from Three.js primitives and BufferGeometry, procedurally generated or canvas-drawn textures, vertex/fragment shader material, and lighting. The whole page stays roughly under 1MB. A scene assembled from primitives and lit with real intention beats a downloaded model that never loads — and a model fetch is exactly what dies inside a funnel embed.
  * CSS FALLBACK IS MANDATORY AND IS CHECKED: delete every <script> mentally and the page must still look finished and designed — never an empty box, never a collapsed section, never a blank hero. Give every canvas/JS-driven region a real static background (on-palette gradient, scrim, image, or typographic composition) underneath it, so a failed CDN, a blocked script, or a device without WebGL degrades to something that still sells. Feature-detect WebGL before initializing and simply leave the fallback in place when it is unavailable.
  * MOTION RESPECTS THE USER: honor prefers-reduced-motion — under it, render ONE static frame (or skip init entirely and keep the CSS fallback) rather than animating.
  * LAZY INIT AND TEARDOWN: never start a render loop on page load. Use an IntersectionObserver to start the loop when the canvas enters the viewport and stop it when it leaves; also stop on document.hidden. A render loop that runs forever on a marketing page is a battery, heat, and performance failure.
  * MOBILE GETS LESS, DELIBERATELY: below 768px either render a reduced scene (fewer particles/objects, lower geometry detail, no post-processing) or fall back to the static CSS treatment. Cap the pixel ratio at Math.min(window.devicePixelRatio, 2) on every device. Never ship the full desktop scene to a phone.
  * NEVER GATE THE WHOLE SCRIPT ON A ONE-SHOT WIDTH CHECK — this is a real failure that has shipped: an early "if (window.innerWidth < 768) return;" at the top of the IIFE runs ONCE at parse time, so a page first laid out in a narrow window (a restored small window, a tablet in portrait, a preview pane, a device that reports a narrow width before layout settles) permanently loses the treatment — the IntersectionObserver and resize listener are never even registered, and widening the window afterwards can never recover it. ALWAYS register the observer and the resize handler unconditionally; decide scene complexity (full / reduced / skip) INSIDE init at the moment it runs, and re-evaluate it on resize. A width check may choose what to build; it must never decide whether the code that can build it exists. The resize handler must be able to START the scene, not only resize an existing one: if it opens with "if (!renderer) return;" then a page loaded narrow can never begin rendering when the window widens, because the renderer it is guarding on is exactly what the narrow load skipped creating. Have resize call init() when the scene is now eligible but not yet built, and only then fall through to the aspect/setSize update.
  * RENDERER SETUP: new THREE.WebGLRenderer({ antialias: true, alpha: true, preserveDrawingBuffer: true }) — preserveDrawingBuffer is required so the scene can be captured for thumbnails and exports. Always handle window resize.
  * NEVER USE: eval, new Function, string-form setTimeout/setInterval, document.cookie, or localStorage/sessionStorage reads. Nothing here needs them and they trip platform security filters.
  * SCOPE JS THE WAY CSS IS SCOPED: query only inside .velpi-page, attach no global listeners beyond resize/scroll/visibilitychange, declare nothing on window, and wrap all of your own code in one IIFE so it cannot collide with GoHighLevel's own scripts on the page.
- THE 3D VOCABULARY — build with these specific techniques, not a vague "make it 3D". Depth comes from LAYERS, not from one spinning object: a parallax stack where foreground, midground, and background move at different rates as the user scrolls; a slow continuous orbit or drift rather than a fast spin; light that establishes a time of day (low warm rake, cool dusk, overcast diffuse) chosen to match the brand's mood; atmospheric depth via fog and a graded sky; subtle drifting particulate (dust, embers, pollen, snow) sized and colored from the palette; a shader gradient mesh that flows slowly; contact shadows so nothing floats unanchored. Scene colors are taken from THIS brand's palette — never default grey clay, never a rainbow, never generic neon-on-black.
- CONVERSION OUTRANKS SPECTACLE, ALWAYS: the 3D exists to make the page feel expensive and hold attention on the offer. It never covers the headline, never delays the CTA, never competes with the primary action for the eye, and never pushes real content below the fold. If a treatment would upstage the conversion path, make it quieter — a restrained scene behind a razor-sharp offer beats a showreel that buries it.
- No position:fixed. Sticky nav is allowed via position:sticky inside .velpi-page.
- DESKTOP-FIRST, MOBILE-SAFE CONTRACT (the page is DESIGNED on a ~1440px canvas; it must NEVER break on a phone):
  ...
  * MOBILE SAFETY — hard requirements, checked before you finish, never sacrificed for the desktop design:
    ...
    - MOBILE NAV: the default and always-safe treatment is a compact bar (~56-64px tall) showing ONLY the logo and that SAME single nav CTA button described above (never a second, mobile-only CTA element) — the full link list REQUIRED by the desktop rule above still exists in the markup and hides by carrying velpi-hide-mobile (do not also write your own @media rule for it) while that one CTA stays visible. This REMOVES nothing from the desktop nav — it only hides the links below 768px. A real JavaScript hamburger toggle is now permitted as an upgrade when the page_mode is "website" and the nav carries 4+ links, but ONLY if built properly: a <button> (never a div) with aria-expanded and aria-controls kept in sync, aria-label="Menu", the panel closing on Escape and on any link activation, focus visibly managed, and the CTA still reachable without opening the menu. If you cannot meet all of that, use the compact bar — a broken or inaccessible menu is far worse than no menu.
