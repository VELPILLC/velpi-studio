...
//
// Lives in the database rather than a repo file because saving happens in the
// RUNNING app: Vercel's filesystem is read-only at runtime, so a file-backed
// library would appear to work locally and silently never persist in prod.
//
// Schema: db/saved_skills.sql (run once in the Supabase SQL editor).
...
export async function listSavedSkills() {
  ...
  try {
    const { data, error } = await supabase
      .from('saved_skills')
      .select('id, name, kind, category, recipe, niches, universal, source_domain, preview, created_at')
      .order('created_at', { ascending: false })
    if (error) return []
    return data || []
  } catch (_) {
    // Best-effort: a page must still build when the library is unreachable.
    return []
  }
}

export async function saveSkillRow(row) {
  if (!supabase) throw new Error('Supabase is not configured.')
  const { data, error } = await supabase
    .from('saved_skills')
    .upsert({ ...row, created_at: row.created_at || new Date().toISOString() }, { onConflict: 'id' })
    .select()
    .single()
  if (error) throw new Error(error.message)
  return data
...
export async function deleteSavedSkill(id) {
  if (!supabase) throw new Error('Supabase is not configured.')
  const { error } = await supabase.from('saved_skills').delete().eq('id', id)
  if (error) throw new Error(error.message)
}
