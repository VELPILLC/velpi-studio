...
// this industry wants. Dependency (css-only vs webgl vs js-library) no longer
// affects the score — see the note in the scorer.
function scoreMotionPresets(presets, analysis, vibeText) {
  ...
  return presets.map(p => {
    ...
    // No dependency bonus: the output contract now allows real JavaScript and
    // pinned CDN libraries, so favouring css-only presets would bias against
    // exactly the advanced treatments this app exists to produce.
    return { p, score }
