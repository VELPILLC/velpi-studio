...
import { chooseFamily, planBorrows, rankWithinFamily, stableHash, CATEGORY_FALLBACK, SECTION_TO_CATEGORY, neededCategoriesFor } from './sectionFamily.mjs'
...
export function pickSectionBlueprints(analysis, {
  ...
  for (const key of keys) {
    ...
    // Rotation is now round-robin across the WHOLE in-family list. This kept
    // its old purpose (six same-category sections shouldn't clone one
    // pattern) but lost its old cost: when the pool spanned 22 codebases,
    // widening the window pulled in foreign design languages. Within a single
    // family it is simply intra-family variation, which is desirable — and
    // repeats can't collide until the list is exhausted.
    const nth = usedPerCategory[cat] || 0
    usedPerCategory[cat] = nth + 1
    const idx = (stableHash(`${domain}:${key}`) + nth) % ranked.length
