...
export function pickSectionBlueprints(analysis, { lockedMap = null, maxRefs = 12 } = {}) {
  ...
  const rankedForCategory = cat => entries
    ...
    .map(e => {
      ...
      // Heroes with a real multi-column desktop composition rank above
      // centered full-height stacks: a centered hero blown up to 1440px is
      // the single biggest "this looks like a phone site on desktop" tell.
      // A decisive niche match (+3) still outranks this bonus, so a
      // centered hero remains reachable when it genuinely fits the niche.
      if (cat === 'hero' && /grid-template-columns/i.test(String(e.reference || ''))) score += 1
      return { e, score }
    })
