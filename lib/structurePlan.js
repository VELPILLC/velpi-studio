...
export function planStructure({ analysis, allStyles, vibe = '', lock = null, manualStyleId = null, guidedDecisions = null }) {
  ...
  if (!styles.length && Array.isArray(lock?.styleIds) && lock.styleIds.length) {
    // Locks written before the one-system rule hold three ids. Keep only the
    // FIRST — that was the niche-matched anchor; the other two were sampled
    // from a top-14 pool. Honoring all three would preserve the old collage
    // for every previously generated business, which is exactly the output
    // this change exists to fix.
    styles = lock.styleIds.map(id => resolveStyleById(allStyles, id)).filter(Boolean).slice(0, 1)
  }
  if (!styles.length) {
    // ONE design system, not three. The old mix took a niche-matched anchor
    // plus TWO more sampled from a top-14 pool — so most of the page's visual
    // language arrived by lottery, and the build prompt was handed three
    // competing "Never" rule sets to fuse. A single committed system is what
    // makes the result read as designed; variety across DIFFERENT businesses
    // still comes from their own niche, palette, logo and copy.
    styles = pickCreativeMix(allStyles, nicheText, vibeText, 1, [], rng)
  }
  ...
  const { refs: sectionRefs, map: sectionMap, family, borrowed } = pickSectionBlueprints(
    { ...analysis, layout: { ...(analysis?.layout || {}), section_order: sectionOrder } },
    {
      lockedMap: lock?.sectionMap || null,
      // The family is part of the regeneration contract: once a business has
      // committed to one, later runs keep it.
      forcedFamily: guidedDecisions?.familyId || lock?.familyId || null,
      forcedMap: guidedDecisions?.forcedMap || null,
    },
  )
