...
export function pickCreativeMix(styles, industryText, vibeText, n = 3, avoidMixes = []) {
  ...
  // Forced variation: if this exact mix was used in a recent generation, swap
  // the wildcard slot with the next-best unused candidate.
  let out = picked.slice(0, n)
  const sig = arr => arr.map(s => s.id).sort().join('+')
  if (avoidMixes.includes(sig(out)) && out.length) {
    const replacement = byTotal.find(x => !out.includes(x.style))
    if (replacement) {
      out = [...out.slice(0, out.length - 1), replacement.style]
    }
  }
  return out
}
