import { isTransientError, backoffDelay, describeError, MAX_CLAUDE_RETRIES } from './claudeRetry.mjs'
...
export async function callClaudeWithUsage({ system, user, images = [], maxTokens = 2000 }) {
  ...
  //
  // Transport resilience: TRANSIENT server errors (429/500/503/529,
  // overloaded_error, rate_limit_error) are retried up to MAX_CLAUDE_RETRIES
  // with exponential backoff + jitter. Non-transient errors (invalid request,
  // auth, permission, context length) are surfaced immediately, unchanged. If
  // all retries are exhausted, the original error is thrown — identical to the
  // prior behavior. Only the transport is affected; requests/responses are the
  // same.
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      if (attempt <= MAX_CLAUDE_RETRIES && isTransientError(err)) {
        const wait = backoffDelay(attempt)
        try { console.warn(`[claude] transient error (${describeError(err)}) — retry ${attempt}/${MAX_CLAUDE_RETRIES} in ${wait}ms`) } catch (_) {}
        await sleep(wait)
        continue
      }
      throw err
