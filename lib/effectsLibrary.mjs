...
//
// `label` / `summary` / `bestFor` are what a HUMAN reads when choosing. They
// are deliberately plain: "floating shapes", "flowing color", "soft drifting
// light". Nobody picking a website for their concrete business should have to
// know what a gradient mesh or size attenuation is, and a jargon label makes
// the choice harder, not more impressive.
//
// `recipe()` is what the MODEL reads, and it is the opposite: the precise
// vocabulary that actually produces depth instead of a spinning object on a
// black square. The words below are load-bearing — parallax layers, orbit,
// time-of-day light, atmospheric fog, contact shadow, procedural texture —
// because a build prompt that says "make it 3D" produces slop, and one that
// names the technique produces a page.
//
// Never leak the second register into the first.

// Appended to every 3D recipe so the depth vocabulary is present on all of
// them rather than remembered case by case.
const DEPTH_VOCABULARY = [
  'DEPTH COMES FROM LAYERS, NOT FROM ONE MOVING OBJECT. Separate the composition into foreground, midground and background and let them move at different rates as the user scrolls (nearer layers travel further) — that parallax separation is what reads as real space.',
  'LIGHT ESTABLISHES A TIME OF DAY. Commit to one — low warm rake, cool dusk, overcast diffuse, hard noon — chosen to match the brand mood, and light every element consistently from it. Flat uniform lighting is what makes 3D look like grey clay.',
  'ATMOSPHERE SELLS DISTANCE: fog or haze graded toward the surface color so far elements dissolve rather than ending at a hard edge.',
  'NOTHING FLOATS UNANCHORED: give any object that sits over a surface a soft contact shadow beneath it.',
  'MOTION IS SLOW AND CONTINUOUS — a drift or an orbit measured in tens of seconds, never a fast spin, never a pulse. If a visitor notices the animation before the headline, it is too strong.',
  'TEXTURE, NOT PLASTIC: vary surface roughness and use procedural or canvas-drawn detail so materials read as concrete, metal, cloth or stone rather than default shiny primitives. Never fetch a texture file.',
].join('\n  ')
...
export const HERO_EFFECTS = [
  {
    ...
    label: 'Soft drifting light',
    intensity: 'subtle',
    summary: 'Tiny points of light drift slowly behind your headline, so the page feels deep and alive instead of flat.',
    bestFor: 'Warm, atmospheric places — restaurants, bars, spas, salons, venues.',
    niches: [
  ...
  },
  {
    ...
    label: 'Flowing color',
    intensity: 'medium',
    summary: 'Your brand colors move behind the headline like slow liquid. Modern and expensive-looking, with no objects on screen.',
    bestFor: 'Software, agencies, consulting, finance — anything selling expertise rather than a physical job.',
    niches: [
  ...
  },
  {
    ...
    intensity: 'bold',
    summary: 'Solid 3D shapes turn slowly in your brand colors, lit like a product photograph. The most obviously three-dimensional option.',
    bestFor: 'Trades and physical work — contractors, builders, automotive, gyms, manufacturing.',
    niches: [
  ...
  {
    id: 'none',
    label: 'Still and serious',
    intensity: 'none',
    summary: 'No motion at all — one strong photograph and confident type. Quiet reads as trustworthy here, where movement would read as a gimmick.',
    bestFor: 'Law, medical, dental, funeral homes, accounting, therapy — anywhere trust matters more than flash.',
