...
// ── Developer Review System — DEV ONLY ───────────────────────────────────────
// The review panel + its APIs exist only in development. Never in production.
// Server gate: not production, OR an explicit DEV_REVIEW=1 override (for a
// preview deploy the operator controls). Client gate mirrors it via
// NEXT_PUBLIC_* which Next inlines at build time.
export function isDevReviewServer() {
  if (typeof process === 'undefined') return false
  if (process.env.DEV_REVIEW === '1') return true
  return process.env.NODE_ENV !== 'production'
}
export function isDevReviewClient() {
  if (typeof process === 'undefined') return false
  if (process.env.NEXT_PUBLIC_DEV_REVIEW === '1') return true
