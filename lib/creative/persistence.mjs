...
// independent rows — they are never interchangeable. Best-effort: returns
// { saved:false } when Supabase/table is unavailable — never throws.

export async function getReview(buildId, viewport, reviewer = 'dev') {
  if (!supabase || !buildId || !viewport) return null
  try {
    const { data, error } = await supabase
...
export async function getReviewsForBuild(buildId, reviewer = 'dev') {
  ...
  if (!supabase || !buildId) return out
  try {
    const { data, error } = await supabase
      .from('creative_reviews')
      .select('*')
...
export async function saveReview(review) {
  ...
  try {
    const row = {
      ...
      flags: review.flags || [],
      notes: review.note || '',
      review_version: review.reviewVersion || 3,
...
export async function listReviewsForExport({ projectId, since, until } = {}) {
  if (!supabase) return []
  try {
    let q = supabase.from('creative_reviews').select('project_id, rating, flags, notes, viewport, created_at')
