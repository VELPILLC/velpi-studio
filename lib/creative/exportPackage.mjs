...
const FIELD_MAP = {
  ...
  developer_review: 'the human developer rating(s) — { mobile, desktop }, each a scores map (overall, layout, images, trust, copy — 1-5) + note, or null if that device view was never rated',
...
function reviewSummary(r) {
  return r ? { scores: r.scores || {}, note: r.note || '' } : null
}
...
//   reviews: { mobile: reviewObj|null, desktop: reviewObj|null } — a build can
//            carry two independent device-scoped ratings
//   project: the raw project row's `data` (projects.data) | null
export function buildSingleArtifact({ cdo, reviews, project, fleetContext, generatedAt } = {}) {
  const rollup = cdo?.rollup || {}
  const pv = promptVersions(cdo)
  const mobileReview = reviews?.mobile || null
  const desktopReview = reviews?.desktop || null
  ...
  return {
    ...
    reviewSchemaVersion: mobileReview?.reviewVersion ?? desktopReview?.reviewVersion ?? null,
    ...
    developer_review: (mobileReview || desktopReview) ? { mobile: reviewSummary(mobileReview), desktop: reviewSummary(desktopReview) } : null,
    html,
    screenshots,
...
//   rows: [{ directive (cdo), reviews: { mobile, desktop } }]
...
export function buildBatchArtifact({ rows = [], metrics = null, selection = {}, includeHtml = false, generatedAt } = {}) {
  const runs = rows.map(({ directive: cdo, reviews }) => {
    const rollup = cdo?.rollup || {}
    const mobileReview = reviews?.mobile || null
    const desktopReview = reviews?.desktop || null
    const base = {
