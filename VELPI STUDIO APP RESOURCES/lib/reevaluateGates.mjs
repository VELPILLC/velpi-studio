...
// Pages now carry real JavaScript — a WebGL hero, scroll behavior, the
// library tags that power them. A "surgical" text fix has no business
// removing any of it, but nothing stopped it: strip every script and the
// result still passed every other gate (valid HTML, same tokens, same vids,
// wrapper intact) while quietly deleting the entire advanced treatment.
export function scriptCountOf(html) {
  return (String(html || '').match(/<script\b/gi) || []).length
}
...
export function checkFixedHtml(before, after) {
  ...
  // Losing a script means losing the WebGL/motion treatment entirely. Gaining
  // one unprompted is just as wrong for a fix that was told to change only
  // what the punch list named.
  if (scriptCountOf(a) !== scriptCountOf(b)) {
    failures.push(`script tags were added or dropped (${scriptCountOf(b)} in, ${scriptCountOf(a)} out)`)
  }
