...
// Creative mixer. Anchors on ONE strong niche match, builds a candidate POOL
// of the top entries by combined niche+vibe score, then samples the remaining
// slots from that pool. The sampling source is injectable: with a seeded rng
// (keyed per-domain by the structure planner) the mix is a pure function of
// the input — the same business gets the same systems on every regeneration,
// while different businesses still land on different blends. Math.random and
// the avoidMixes rejection list remain only for legacy callers.
export function pickCreativeMix(styles, industryText, vibeText, n = 4, avoidMixes = [], rng = null) {
  ...
  // Anchor: sampled among the strongest niche matches instead of always the
  // single top scorer — a fixed #1 anchor made every business in a niche
  // converge on one look. With a seeded rng the anchor is still a pure
  // function of the domain, so per-business consistency is unchanged; only
  // cross-business variety improves.
  const random0 = rng || Math.random
  const nicheMatches = byNiche.filter(x => x.nicheScore > 0)
  const anchorPool = nicheMatches.slice(0, Math.min(3, nicheMatches.length))
  const anchor = anchorPool.length
    ? anchorPool[Math.floor(random0() * anchorPool.length)].style
    : (byTotal[0]?.style || styles[0])
  if (!anchor) return []
  ...
  const sample = () => {
    ...
    while (picks.length < n && bag.length) {
      // Mildly front-weighted (1.15, was 1.6 — the heavy bias pulled every
      // business toward the same top-of-pool entries); deterministic when
      // seeded, so the same domain still resolves the same mix.
      const idx = Math.floor(Math.pow(random(), 1.15) * bag.length)
