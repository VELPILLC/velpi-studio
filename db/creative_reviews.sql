...
-- v2: flat rating model. `run_id` is now "this build's id" whether or not the
-- CIL ever produced a Creative Directive for it (reviews no longer depend on
-- CIL). Additive only — old flag/scores/tags/review_version=1 rows are left in
-- place untouched; the app only reads/writes rating/flags/notes going forward.
alter table creative_reviews add column if not exists rating text;              -- 'love' | 'okay' | 'needs_work' | null
